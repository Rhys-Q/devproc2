...
- `metadata` 支持 `grid`、`block`、`shared_memory_bytes`、`sm_arches`、`include_dirs`、`extra_nvcc_flags`、`compile_options`、`kernel_name`、`effect`。
...
后续迁移 Pi0.5 fast path 时按以下规则执行：

- 新增 CUDA fast path 不再直接调用 `dp.call_dps_kernel()`。
- 简单 kernel 先迁移为 `dp.empty() + dp.cuda_call(...)`，例如 cast、bias add、Euler update。
- 多输出 kernel 用多个 `dp.empty()` 输出并传入同一个 `cuda_call`。
...
- 覆盖测试见 `tests/compiler/test_cuda_custom_call.py`。
