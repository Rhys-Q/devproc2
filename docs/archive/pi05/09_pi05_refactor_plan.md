...
迁移期可以短暂保留薄兼容层；最终结构不保留 `modules.py`，Pi0.5 模型类的正式入口是 `devproc2.models.pi05.model` 或包级 `devproc2.models.pi05`。
...
Hard constraints:
- Preserve public imports during migration with thin compatibility wrappers, then remove those wrappers once canonical paths are in place.
...
```bash
pytest tests/compiler/test_pi05_fast_modules.py tests/compiler/test_pi05_artifact.py tests/compiler/test_pi05_weight_package.py tests/compiler/test_pi05_nn_frontend.py
rg "dp\\.cuda_call|dp\\.call_dps_packed" python/devproc2/models/pi05/model.py
rg "if .*target|config\\.target|sm89|thor" python/devproc2/models/pi05/model.py
```
...
rg "dp\\.tensor_view|tensor_view\\(" python/devproc2/models/pi05/model.py
...
本阶段做模型目录收敛；迁移期 wrapper 只允许短暂存在，最终清理时删除。

- 将 `modules.py` 收敛到 `model.py`，最终删除 `modules.py`。
...
assert new.PI05DenoiseLoop is not None
assert importlib.util.find_spec("devproc2.models.pi05.modules") is None
PY
```
