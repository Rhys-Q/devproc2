# Vendored: Flash-Attention 2 forward kernels (BF16, SM80)
...
- Subset:    `csrc/flash_attn/src/` headers + fwd BF16 SM80 kernel
             instantiations for head-dim buckets 96 and 256. bwd, CK (AMD) path, Hopper FA3,
             alibi/rotary/dropout runtime paths (headers kept for
             compile; p_dropout=0 in inference means inert), and the
             upstream framework-coupled `flash_api.cpp` pybind wrapper is excluded
...
Three framework-decoupling patches in `flash_attn/`:
