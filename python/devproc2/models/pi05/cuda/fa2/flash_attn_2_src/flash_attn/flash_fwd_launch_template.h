...
// behaviour (abort on CUDA error) without pulling in a framework wheel.
#include <cstdio>
#include <cstdlib>
...
#define C10_CUDA_CHECK(expr) do {                                          \
    ...
    if (_fa2_err != cudaSuccess) {                                         \
        fprintf(stderr, "Devproc2 Pi0.5 FA2 CUDA error %s:%d: %s\n",      \
...
#define C10_CUDA_KERNEL_LAUNCH_CHECK() do {                                \
    ...
    if (_fa2_err != cudaSuccess) {                                         \
        fprintf(stderr, "Devproc2 Pi0.5 FA2 kernel launch error %s:%d: %s\n", \
                __FILE__, __LINE__, cudaGetErrorString(_fa2_err));         \
        std::abort();                                                      \
    }                                                                      \
} while (0)
