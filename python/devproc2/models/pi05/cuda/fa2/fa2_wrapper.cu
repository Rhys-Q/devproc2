// ================================================================
// Devproc2 Pi0.5 raw-pointer entries for vendored Flash-Attention 2
//
// Replaces the upstream framework wrapper with two C-linkage entries:
//   devproc2_pi05_attention_fa2_fwd_fp16   — elem_type = cutlass::half_t
//   devproc2_pi05_attention_fa2_fwd_bf16   — elem_type = cutlass::bfloat16_t
...
[[noreturn]] static void fa2_not_built(const char* which, int v) {
    fprintf(stderr,
        "devproc2_pi05_attention_fa2: %s=%d was not compiled into this build. "
                           ...
                           cudaStream_t stream)
{
    ...
    if (head_dim > 0 && head_dim <= 96) {
        ...
        fprintf(stderr,
            "devproc2_pi05_attention_fa2_fwd: unsupported head_dim=%d "
...
{                                                                                \
    fprintf(stderr,                                                              \
        "devproc2_pi05_attention_fa2: " DTYPE_STR " entry was not compiled. "   \
        "Rebuild with -DFA2_DTYPES=\"fp16;bf16\" to enable it.\n");              \
    std::abort();                                                                \
}

#ifdef FA2_HAS_FP16
DEFINE_FA2_ENTRY(devproc2_pi05_attention_fa2_fwd_fp16, cutlass::half_t,    false)
#else
DEFINE_FA2_STUB(devproc2_pi05_attention_fa2_fwd_fp16,  "fp16")
#endif

#ifdef FA2_HAS_BF16
DEFINE_FA2_ENTRY(devproc2_pi05_attention_fa2_fwd_bf16, cutlass::bfloat16_t, true)
#else
DEFINE_FA2_STUB(devproc2_pi05_attention_fa2_fwd_bf16,  "bf16")
#endif
